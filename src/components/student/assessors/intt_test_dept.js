...
class StudentAssessorITD extends Component {
  ...
  componentWillMount() {
    axios.get(`/assessor_api/dept/ITD`)
    .then((data) =>{
        if (data.status === 200){
          console.log(data);
          this.setState({
  ...
  render() {
    const assessor_content = (
      <div className='row'>
        {this.state.assessors.map((assessor) => 
          <div className='col'>
            <Card style={{ width: '18rem' }} key={assessor.id}>
              <Card.Body>
    ...
    const no_content = (
      <div className='row'>
        <div className='col' style={{width:'100%', textAlign:'center'}}>
          <p>No Assessor For Intt Test Department</p>
        </div>
      </div>
    )
    ...
    return (
      <div className="container-fluid">
        <Card>
          <Card.Header>Intt Test Dept. Assessors</Card.Header>
          <Card.Body>
...
export default StudentAssessorITD;
